...
/**
 * Which offices sign each document.
 *
 * The company signs its own papers, not whichever admin happened to open the page. An offer letter
 * carries the same signatures whether it was generated by the sales admin at 9am or the tech admin
 ...
 *
 * **Both officers sign everything the company issues**, by the company's own instruction. Two
 * signatures on a letter is a stronger document than one: an employee showing an experience letter
 * to a future employer, or a settlement statement to a bank, is handing over something that two
 * named officers stood behind.
 *
 * The policy acknowledgement is the sole exception, and not an oversight. It is not the company
 * making a promise — it is the employee confirming they have read the rules — so it is signed by
 * the employee and countersigned by the company as having received it.
 */
export const BOTH_OFFICERS: OfficerKey[] = ["ceo", "cto"];

export const DOCUMENT_SIGNATORIES: Record<HrDocumentType, OfficerKey[]> = {
  offer_letter: BOTH_OFFICERS,
  appointment_letter: BOTH_OFFICERS,
  nda: BOTH_OFFICERS,
  policy_acknowledgement: BOTH_OFFICERS,
  confirmation_letter: BOTH_OFFICERS,
  probation_extension: BOTH_OFFICERS,
  increment_letter: BOTH_OFFICERS,
  promotion_letter: BOTH_OFFICERS,
  show_cause_notice: BOTH_OFFICERS,
  warning_letter: BOTH_OFFICERS,
  resignation_acceptance: BOTH_OFFICERS,
  relieving_letter: BOTH_OFFICERS,
  experience_letter: BOTH_OFFICERS,
  full_final_settlement: BOTH_OFFICERS,
};

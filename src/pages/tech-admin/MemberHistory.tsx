...
import { ArrowLeft, ExternalLink, FileText, Image, Trash2, Loader2 } from "lucide-react";
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <button onClick={() => navigate(-1)} className="w-8 h-8 rounded-lg bg-accent flex items-center justify-center text-muted-foreground hover:text-foreground transition-colors">
            <ArrowLeft size={16} />
          </button>
          <div>
            <h1 className="font-display text-2xl font-bold text-foreground">{member?.name || "Member"}'s History</h1>
            <p className="text-muted-foreground text-sm mt-0.5">{submissions.length} total submissions · {member?.email}</p>
          </div>
        </div>

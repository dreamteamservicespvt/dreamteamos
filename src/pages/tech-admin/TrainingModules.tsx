...
export default function TechTrainingModules() {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      for (let i = 0; i < pendingFiles.length; i++) {
        ...
        uploadedFiles.push({ name: file.name, url, type: getFileType(file) });
      }
      ...
      setModules((prev) => [{
        id: docRef.id, title: title.trim(), purpose: purpose.trim(),
        files: uploadedFiles, addedBy: currentUser?.uid || "",
        department: "tech", createdAt: { seconds: Date.now() / 1000 },
      }, ...prev]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between gap-2">
        <div>
          ...
          <p className="text-muted-foreground text-xs md:text-sm mt-1">Training resources for your tech team</p>
        </div>

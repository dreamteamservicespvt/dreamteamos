...
export default function TechMemberTraining() {
  const [modules, setModules] = useState<TrainingModule[]>([]);
  const [loading, setLoading] = useState(true);
  const [selected, setSelected] = useState<string | null>(null);

  useEffect(() => {
    const q = query(collection(db, "training_modules"), where("department", "in", ["tech", "all"]));
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">Tech resources and learning modules</p>
      </div>

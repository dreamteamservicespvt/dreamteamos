...
import { ExternalLink, FileText, Image, Trash2, Loader2 } from "lucide-react";
...
export default function MyHistory() {
  ...
  const dateFiltered = dateStr ? submissions.filter((s) => s.date === dateStr) : submissions;
  const filtered = filter === "all" ? dateFiltered : dateFiltered.filter((s) => s.status === filter);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-display text-2xl font-bold text-foreground">Work History</h1>
          <p className="text-muted-foreground text-sm mt-1">{dateFiltered.length} submissions{dateStr ? ` on ${format(selectedDate!, "dd/MM/yyyy")}` : ""}</p>
        </div>
      ...
      <div className="flex gap-1.5 flex-wrap">
        {["all", "pending", "approved", "rejected"].map((f) => (
          ...
          >
            {f} {f !== "all" ? `(${dateFiltered.filter((s) => s.status === f).length})` : `(${dateFiltered.length})`}
          </button>
        ))}
      </div>

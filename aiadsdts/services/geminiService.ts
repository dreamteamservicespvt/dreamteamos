...
Return only the repaired ${segmentCount} clip lines.`;
  ...
  COMMERCIAL PROFESSIONAL RULE: When Model Attire = Professional (Premium Beige/Pastel Suit), treat the output as a premium corporate commercial hero shot only — never festival-themed, never celebratory, never saree-led, never decorated with cultural props. In this suit branch the woman must still look like a Vogue India / Tanishq / Lakme campaign-tier Indian beauty with a real warm smile, alive direct eyes, visible skin truth, and semi-jewellery only. Ban plain receptionist look, HR portrait look, employee ID-photo look, LinkedIn headshot energy, and generic office-worker prettiness.
...
- The scene should look like a REAL photo taken at the ACTUAL business with their products on display`
  ...
  PROFESSIONAL ATTIRE RULE: When ATTIRE = Professional, build the frame in a bright contemporary corporate-facing business zone with premium beige/pastel suit styling, semi-jewellery only, and zero festival cues. The suit woman must be strictly 20 to 25 years old, distinctly Indian, actress-level beautiful, smiling warmly, and impossible to confuse with a generic employee portrait. For Clip 1, use the exact folded-hands hero pose at the waist like the traditional and festival anchor frame. From Clip 2 onward, the hand position and pose must change according to that clip's exact voice-over script and location.
